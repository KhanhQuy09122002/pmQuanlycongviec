...
<div class="row text-center" style="width: 100%">
    <div class="col-md-12" style="width: 100%"> 
        ...
        <table id="table-tieu-de-1" style="width: 100%">
    		<tr>    		
    			<td colspan="2">
    				<span class="phieu-h1">BÁO CÁO DOANH THU THEO HÀNG HÓA</span>
    			</td>    			
    		</tr>
    		<tr>
    		...
    		<tr>    			
    			<td colspan="2" style="text-align: left;"><span>Nhân viên: <?= $byuser ? User::findOne($byuser)->ho_ten : 'Tất cả' ?></span></td>
    		</tr> 		
    	</table>
        ...
        <table class="table-content" style="width: 100%; margin-top:5px;">
        	<thead>
        		<tr>
        			<th width="20">STT</th>
        			<th width="60">Mã hàng hóa</th>
        			<th width="120">Tên hàng hóa</th>
        	...
        	<?php 
        	    ...
        	    foreach ($model as $indexCt => $item){
        	        $tongTien += $item->tongTienSanPham;
        	        $tongCK += $item->tongChietKhauSanPham;
            ?>
            <tr>
            	<td style="text-align:center"><?= ($indexCt+1) ?></td>
            	<td style="text-align:center"><?= $item->hangHoa->ma_hang_hoa ?></td>
            	<td style="text-align:center"><?= $item->hangHoa->ten_hang_hoa ?></td>
            	<td style="text-align:right"><?= number_format($item->tongSoLuongSanPham) ?></td>
            	<td style="text-align:right"><?= number_format($item->tongTienSanPham) ?></td>
            	<td style="text-align:right"><?= number_format($item->tongChietKhauSanPham) ?></td>
            	<td style="text-align:right"><?= number_format($item->tongTienSanPham - $item->tongChietKhauSanPham) ?></td>
            </tr>
            <?php }} ?>	
            <tr style="font-weight: bold">
            	...
            	<td align="right"><?= number_format($tongTien - $tongCK) ?></td>
            </tr>
            </tbody>
        </table>

        ...
        <div class="col-md-3">
            ...
            <?= $form->field($model, 'id_khach_hang')->widget(Select2::classname(), [
                ...
                'pluginOptions' => [
                    ...
                    'ajax' => [
                        'url' => '/khachhang/khach-hang/search',
                        'dataType' => 'json',
                        'delay' => 250,
                ...
                /* 'pluginEvents' => [
                    "select2:select" => "function(e) {
                        console.log('Bạn đã chọn: ' + e.params.data.text);
                    }",
                ] */
            ])->label(false); ?>
        </div>
...
function InHoaDon(){
	...
	$.ajax({
        type: 'post',
        url: '/banhang/hoa-don-ban-hang/get-phieu-xuat-kho-in-ajax?idHoaDon=<?= $model->id ?>',
        //data: frm.serialize(),
        success: function (data) {
            console.log('Submission was successful.');
            console.log(data);            
...
function getKhachHangAjax(idkh){
    $.ajax({
        type: 'post',
        url: '/khachhang/khach-hang/get-khach-hang-ajax?idkh=' + idkh,
        //data: frm.serialize(),
        success: function (data) {
            console.log('Submission was successful.');
            console.log(data);            
...
/* $(document).on('select2:select', '#khach-hang-dropdown', function (e) {
    console.log('SELECT fired');
    console.log('Data:', e.params.data); // ← Đây kiểm tra nếu bị undefined
}); */

function runFunc(sendVal){
	var url = '/khachhang/khach-hang/refresh-select2?selected=' + sendVal;
	$.ajax({
        url: url,
        method: 'GET',
